...
- **Phase 1 core loop is wired end-to-end, has been through substantial live-testing and debugging since this addendum was first written, but is still not validated as reliably working.** MusicXML + photo import, OSMD rendering with automatic beaming (now correctly triggered for both import paths, not just photo — see below), score-following (`practice/cursor.ts`'s `ScoreFollower`, driven by both `audio/onsetDetector.ts`'s spectral-flux onset detection and a second `practice/implicitOnsetWatcher.ts` trigger added specifically to catch legato/slurred note changes that produce no attack transient), a live in-tune/out-of-tune indicator, and a post-session practice review summary (`practice/reviewSummary.ts`) all call into each other in `src/App.tsx`. Real, live-testing-confirmed bugs found and fixed since this addendum was first written: a too-permissive pitch-confidence threshold letting background noise register as a confident note; MusicXML (not just photo) imports never getting reliable beam rendering (beam recomputation was gated on import source instead of on whether the source XML actually had beam data); tied and same-pitch-slurred notes stalling the cursor entirely (a tie/same-pitch slur produces no attack and no pitch change, so neither note-change trigger could ever fire for it); and the practice session's visual cursor drifting to the final barline instead of the session actually ending. A more ambitious score-following redesign — continuous best-fit alignment against a sliding window of recently-played pitches, instead of the discrete onset-gated state machine — was implemented, passed its offline test suite, then live-tested and found to get stuck and sometimes jump to a random note; it was reverted back to the discrete design. See `CLAUDE.md`'s score-following section for the full implementation-level account, including the specific tradeoffs (e.g. vibrato-safety tuning that trades away reliable half-step-legato detection via the implicit trigger) baked into the current design. Net: meaningfully more solid than "wired but early-stage," but still not a confirmed "actually works well end-to-end" pass — heavily-slurred pieces specifically still need more live verification.
